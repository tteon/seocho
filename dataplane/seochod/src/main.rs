...
fn props_send(mut props: HashMap<String, Value>, workspace_id: &str, source_id: &str, writer_ts: f64, receipt: Option<&SemanticReceipt>) -> Result<HashMap<String, ValueSend>, String> {
    ...
    if let Some(receipt) = receipt {
        props.insert("_rdf_bundle_sha256".into(), Value::String(receipt.rdf_bundle_sha256.clone()));
        props.insert("_rdf_data_graph_sha256".into(), Value::String(receipt.rdf_data_graph_sha256.clone()));
        props.insert("_agent_profile_sha256".into(), Value::String(receipt.agent_profile_sha256.clone()));
        props.insert("_projection_receipt_sha256".into(), Value::String(receipt.projection_receipt_sha256.clone()));
    }
...
fn project(driver: &Driver, request: Request) -> Response {
    ...
    if required && receipt.is_none() { return failure("canonical projection requires a semantic governance receipt".into()); }
    if let Some(value) = receipt {
        if value.schema_version != "seocho.canonical_projection_receipt.v1" || ![&value.rdf_bundle_sha256, &value.rdf_data_graph_sha256, &value.agent_profile_sha256, &value.projection_receipt_sha256].iter().all(|digest| digest.len() == 64 && digest.chars().all(|c| c.is_ascii_hexdigit())) {
    ...
    for node in request.nodes.unwrap_or_default() {
        ...
        let props = match props_send(node.properties, &workspace_id, &source_id, writer_ts, receipt) { Ok(v) => v, Err(e) => { response.errors.push(format!("node {}: {e}", node.id)); continue; } };
    ...
    for rel in request.relationships.unwrap_or_default() {
        ...
        let props = match props_send(rel.properties, &workspace_id, &source_id, writer_ts, receipt) { Ok(v) => v, Err(e) => { response.errors.push(format!("relationship: {e}")); continue; } };
