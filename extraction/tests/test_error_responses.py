...
class TestStructuredErrorResponses:
    def test_configuration_error_returns_400(self, client):
    ...
    def test_infrastructure_error_returns_502(self, client):
        response = client.get("/raise/infra")
    ...
    def test_pipeline_error_returns_422(self, client):
        response = client.get("/raise/pipeline")
        assert response.status_code == 422
        data = response.json()
        assert data["error"]["error_code"] == "ExtractionError"

    def test_base_seocho_error_returns_500(self, client):
        response = client.get("/raise/base")
    ...
    def test_x_request_id_in_error_response_headers(self, client):
        response = client.get("/raise/config")
