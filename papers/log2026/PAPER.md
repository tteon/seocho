...
The cost of leaving them separate is concrete, and this paper measures it. A question whose evidence sits in two teams' graphs leaves a system two bad options. Ask every graph, and it pays one agent call per view, widens the policy surface, and amplifies whatever is wrong in any of them; in our controlled tests, unrestricted broadcast leaked a protected field in 26 of 31 cases. Ask one fixed graph, and the required fact is simply absent. Neither is acceptable where answers must be attributable and access-controlled, which is exactly the setting these graphs exist to serve. Engines now let an authorized application read across databases [16], so what remains is not an execution problem but a semantic one: deciding which views a question needs.
