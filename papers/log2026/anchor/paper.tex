...
schema guidance \emph{reduces} cross-model agreement on entity names and does
not increase coverage of answer-bearing facts; its measurable contribution is
\emph{detectability} --- schema violations become checkable. (2)~The standard
...
source token yields 1.7--2.8$\times$ more comparable facts and reveals that a
...
serialized graphs. An \emph{evidence-conditional} evaluation --- separating
memorization, honest abstention, and grounded answers --- reverses this on
two of three models, and attributes the residual gap to model-specific
