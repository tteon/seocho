...
An organization that answers questions from its own documents rarely keeps one
knowledge graph. Accounting, risk, and governance teams each build their own,
because each owns its extraction rules, its provenance, and the authorization
that decides who may read it. Those boundaries are the reason the graphs are
trusted, so merging them is not a neutral optimization: it discards the
ownership that made each view auditable, and it creates read paths that policy
was written to prevent.

The cost of leaving them separate is concrete, and this paper measures it. A
question whose evidence sits in two teams' graphs leaves a system two bad
options. Ask every graph, and it pays one agent call per view, widens the
policy surface, and amplifies whatever is wrong in any of them. In a set of 31
synthetic fault-injection cases, unrestricted broadcast leaked a protected field
26 times; these are deliberately easy stress tests and not natural-query rates,
but the exposure they demonstrate is the exposure the design has to avoid. Ask one fixed graph, and the required fact is simply absent. Neither is
acceptable where answers must be attributable and access-controlled, which is
exactly the setting these graphs exist to serve. Engines now let an authorized
application read across databases~\citep{memgraph2026}, so what remains is not
an execution problem but a semantic one: deciding which views a question needs.

\paragraph{The problem, stated precisely.} Given a question and several
authorized graphs, decide which of them the answer actually needs, before
generating anything. A system must distinguish four cases: one graph suffices,
a second supplies a missing fact, two must be compared because their claims
conflict, or no authorized combination can answer and the system should decline.
Success is not a better sentence; it is choosing the smallest authorized set
that covers the question, and knowing when no set does.

\paragraph{What we built.} Slot-and-Divergence Coalition Routing (SDCR)
identifies the facts a question requires, selects the smallest authorized set of
category agents that can supply them, and passes only typed evidence to a
supervisor. Missing slot coverage and comparable-fact conflict are hard
conditions. Graph-divergence statistics only rank candidates that already pass.
Figure~\ref{fig:decision} states the whole rule.

\paragraph{What happened.} Two of the design's three premises held and the third
did not. Isolated views are genuinely different observations once identity is
governed, and selective verification does keep conflicting or protected evidence
out of answers. But graph divergence turned out to be unusable as a trigger, and
the component we had assumed was easy is the one that failed: on blind-validated
questions the router found both required views four times out of thirteen. At a
fixed evidence budget it returned .045 slot recall where random selection at the
same coverage already gives .134. Substituting three different answer models
reproduced the deficit rather than removing it, which locates the failure in
selection and not in generation.

\paragraph{The insight, and why it transfers.} Conditioning on the router's own
outcome explains the shortfall and generalizes past this system. Where selection
succeeded, the learned router beat a term-frequency baseline (.147 against
.083); where it missed, it scored exactly .000, because an uncovered selection
serves no evidence at all. When agents own disjoint slices of the corpus, a
selection error does not degrade the evidence, it empties it. Expected quality is
therefore governed by the miss rate and by what gets served on a miss, not by how
good the hits are. Serving a capability-ranked team whenever coverage is
incomplete raised retrieval from .045 to .152 and improved answers for every
model we tried. The practical implication is uncomfortable but cheap: a router
over partitioned stores should ship its fallback before it ships a better
selector, and should be reported on coverage and on miss behavior rather than on
average score alone.

\paragraph{What we deliberately do not claim.} The revised answer set contains 13
model-constructed, model-validated questions with 4 routing successes. That is
too small for a confirmatory answer comparison, and we do not offer one. Even
with the fallback, the coalition never overtakes simply centralizing retrieval;
every interval against centralization contains zero. We use the set to establish
where the pipeline breaks, and we report the audit that invalidated our own
earlier and more favorable construction. Section~\ref{sec:setup} states the
sampling and screening defects we found in our own artifacts, including a
heuristic issuer extractor that must be rebuilt before the candidate pool is
reused.

\section{Related Work}
Knowledge-graph QA combines retrieval and symbolic structure, including execution-guided reasoning, graph-pattern answering, and GNN--LLM collaboration~\citep{ren2021lego,cucumides2025unravl,liu2025dual}. GraphRAG systems increasingly use agent collaboration~\citep{capozzi2026agentic,xu2025openworld}, while language-agent systems can be represented as interaction graphs~\citep{zhuge2024gptswarm}. These methods typically assume a shared graph, or they focus on reasoning after retrieval. Our specialist's isolated ontology-specific graph is instead a private observation.
...
\label{eq:gate}
\end{equation}
PPR divergence does not appear in Eq.~\ref{eq:gate}. If $M(q)=0$, SDCR returns $\{i^*\}$. Otherwise it takes the smallest authorized coalition covering every required slot, with two independent views required for a conflicting slot, and abstains when the feasible set is empty. Divergence and typed-path terms break ties only among minimum-cost coalitions; Appendix~\ref{app:routing} gives that objective.

Every action receives the same total evidence budget, and entity resolution is identifier-first and precedes centrality, so degree only ranks entities that resolution has already accepted. Appendix~\ref{app:routing} gives the resolution rule, the PageRank settings, and the per-arm node caps.
...
\paragraph{Limitations.} The revised set contains 13 model-constructed and model-validated queries with only four routing successes, so it cannot support a confirmatory answer comparison and we do not offer one; persona reviews are correlated model simulations, not independent human annotation. The issuer extractor is a heuristic that provably merges distinct companies in 3 of 240 candidates and probably in 2 more, so the wider pool must be rebuilt before reuse. FinDER categories proxy organizational teams rather than observed ownership and authorization; source gold can contain generic inference; token F1 remains lexical; and the exact-token tokenizer is a fixed reproducibility tokenizer and not the serving provider's internal one. MiniMax often violates the JSON contract, and because routing failures were booked separately from schema failures the schema-failure column is not comparable between the routed and fallback arms. The fallback inherits the 13-case sample limits, and two of its three answer intervals pin at zero because the arms are identical on the four covered cases. Adversarial results are synthetic, the mediation gate has two cases per category, and dropout is tested at capability level rather than end to end. Finally, the system assumes an externally correct authorization service and does not solve policy administration.

\paragraph{Reproducibility.} Artifacts freeze case identifiers, splits, graph and ontology hashes, identity and routing receipts, prompt text, blinded reviews, evidence bundles, schema failures, and case-level metrics; projections are non-destructive and paid calls resume-safe. The evaluation set is the 13-query core. A 120-query held-out expansion pool is marked \texttt{pending\_construct\_validation} and contributes to no reported number; as frozen it is \emph{not} disjoint from the core, so we release a v2 manifest with the overlapping queries removed. Appendix~\ref{app:data} onward records the construction, descriptors, personas, routing, serialization, and failed protocol versions, and the anonymous artifact separates zero-cost replay from optional paid reproduction.
