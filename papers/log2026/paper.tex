...
Because independent financial annotation is costly, we use a blinded heterogeneous LLM-judge panel only as an expert proxy. FinanceBench demonstrates evidence-linked financial QA evaluation~\citep{islam2023financebench}, while JUDGE-BENCH shows that judge agreement varies by task and evaluator expertise~\citep{bavaresco2024judgebench}. We report panel agreement, leave-one-role-out stability, and control-set calibration, and never describe panel consensus as human agreement~\citep{zhou2025jetts,kim2024prometheus2,chang2025llmjudge}.

\paragraph{Scope against agent-memory frameworks.} LightRAG, Graphiti, and Mem0 are neighboring infrastructure rather than baselines: they answer how to store and retrieve agent memory, not how to route a query across isolated organizational views under an authorization and evidence budget. Their default indexing and update policies would change the observation space itself, so an end-to-end numerical comparison would be confounded. We compare controlled routing policies on frozen category snapshots instead, and would adopt an external implementation only if it preserved the same snapshots, authorization mask, evidence cap, and evaluator.
...
\caption{Counts at each layer. Extraction expands 5,703 records into isolated provider--case workspaces and then into a large raw graph; normalization and the qualified read-only projection contract it again. Units differ per row and are labelled, because the rows are not subsets of one another.}
\label{fig:lineage}
\end{figure}

FinDER contains 5,703 questions across eight financial categories. Four generation models ground every case under baseline and duplicate-aware survivorship profiles. The survivorship snapshot has 22,812 provider--case workspaces, 523,092 nodes, and 1,211,526 relations; collapsing repeated normalized observations within category views yields 187,601 semantic nodes and 166,147 typed edges. A separate $4\times5\times4\times16$ prompt--ontology--model factorial contains 1,280 completed cells with no errors. Qualified answer cases are non-destructively projected into eight LoG-specific databases holding 8,313 nodes and 20,011 relations (Figure~\ref{fig:lineage}). The eight category views are organizational observation regimes, not random partitions, and Figure~\ref{fig:category} shows that no single statistic orders them: Risk has the highest mean degree and the lowest normalized PageRank entropy, while Footnotes has the most nodes and one of the lowest mean degrees. This is why raw centrality cannot be compared across views as a routing signal.
...
\paragraph{Defect 1: the issuer label is a heuristic, not a resolved identifier.} Candidate-time issuer extraction takes the last uppercase two-to-five letter token that is not in a small stop list. This is \emph{not} the identifier-first resolution of Section~\ref{sec:observation}: it selects trailing acronyms, so an accounting abbreviation can become an ``issuer'' and two different companies can be paired under one label, which breaks the same-issuer premise that the pairing, the issuer-disjoint split, and every issuer-clustered interval rely on.

We quantified the damage by re-deriving the pool with a validated extractor that reuses the paper's own identifier policy --- a token qualifies only as an accepted \texttt{ticker:*} in the frozen identity registry (537 tickers), and a question naming more than one accepted ticker is treated as a cannot-link and dropped. Three of the 240 candidates provably pair two different companies, all labelled \texttt{EPS}: a Cintas with a United Airlines question, a Cisco with a United Airlines question, and a Cisco with a Cintas question. Two further candidates labelled \texttt{IP} pair Synopsys with Keysight by manual reading, which the registry cannot adjudicate because neither ticker is in its accepted set. Fifteen issuer labels are not accepted tickers, but that is partly registry coverage rather than mislabelling --- several (\texttt{BMY}, \texttt{FDX}, \texttt{KLA}) are genuine tickers absent from a registry built from repeated entities. Re-deriving the pool changes 33 of 240 candidates and shifts the split from 51/189 to 57/183.

The evaluated core is materially unaffected. We inspected all 26 component questions of the 13 cases by hand: each names its assigned issuer exactly once. Twelve of the 13 survive the validated extractor; the exception, \texttt{TT}, drops only because Trane Technologies is absent from the registry, not because the label is wrong. We therefore report the frozen chain and treat the corrected pool as the input to a future run, because re-deriving the chain requires the paid five-role persona gate. The corrected pool and a core-disjoint expansion pool are released as artifacts.

\paragraph{Defect 2: our own construction did not survive a blind audit.} We audited construct validity without showing reviewers graph outputs, answer scores, or author decisions. Two MARA reviewers agree on 88.6\% of the 35 candidates but their consensus agrees with the author on only 77.1\%. Four AGY role personas and a Codex chair examined nine disputed or audit-sampled items and rejected all nine, because many prompts concatenate two topics without forcing a joint decision and some source gold contains generic inference. We therefore retain the 28-case result only as a failed exploratory construction audit. Codex rewrote the 28 pairs into atomic integrative questions using only source gold, and a separate five-role blinded model-persona gate retained 13 under a three-of-five rule requiring valid atomic slots and supported cross-view conclusions. Persona outputs are correlated model simulations; no panel vote is labelled a human annotation. We therefore report the panel as a calibrated expert-proxy: role agreement, majority stability, paraphrase and order stability, and calibration on FinDER-native and synthetic controls. The complete schema and claim boundary ship with the artifact as \texttt{LLM\_JUDGE\_PROTOCOL.md}; Appendix~A.2 states the gate itself.
...
SDCR & \textbf{.981} & \textbf{.975} & 1.32 & \textbf{.056} & 1.000\\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
\multicolumn{4}{l}{\emph{Conditioned on the router's own outcome (slot token recall)}}\\
...
\paragraph{The repair reaches the answer, but stops at centralization.} Serving the fallback evidence to the same three models with the same harness (Table~\ref{tab:fallback-answers}) raises slot F1 for every model relative to routed SDCR, and the issuer-clustered interval excludes zero for GPT-OSS ($+.067$, $[.008,.124]$). For DeepSeek ($+.027$) and MiniMax ($+.011$) the lower bound pins at exactly $.000$ for a mechanical reason worth stating: the two arms serve identical evidence on the four cases the router covered, so those per-case deltas are exactly zero and a clustered resample can contain only such cases. Against centralized single and qualified-view broadcast, however, all six intervals contain zero --- the largest point estimate in either direction is $-.028$. The fallback therefore recovers what the routing failure was destroying and no more. This is the answer-level claim the retrieval replay could not make, and it leaves the paper's central negative result intact: at $n=13$ there is still no measurable coalition advantage over simply centralizing.
...
\subsection{Is answer synthesis the cause?}
...
\midrule
\multirow{3}{*}{DeepSeek-V3.1} & Centralized & .062 & .154 & .123 & .000\\
 & Broadcast & .072 & .108 & .108 & .000\\
 & SDCR & .044 & .031 & .065 & .000\\
\midrule
\multirow{3}{*}{GPT-OSS-120B} & Centralized & .133 & .284 & .241 & .077\\
 & Broadcast & .128 & .313 & .245 & .077\\
 & SDCR & .038 & .031 & .063 & .000\\
\midrule
\multirow{3}{*}{MiniMax-M2.7} & Centralized & .012 & .000 & .019 & .923\\
...
\subsection{Do graph-construction changes even reach the answer?}
Not measurably, in the calibration gate we can afford to run exhaustively. The 16-case, 256-cell contrast crosses two prompts, two ontologies, and four graph-generation models. Mean retrieval token recall is .352 and retrieval numeric recall .886; the fixed DeepSeek answerer has no schema failures and mean token F1 .286. A case-blocked standardized serial path tests treatment $\rightarrow$ nodes-plus-relations $\rightarrow$ retrieval recall $\rightarrow$ answer F1. The serial indirect estimate is $.00006$ for prompt (95\% CI $[-.00046,.00251]$) and $-.00001$ for ontology ($[-.00099,.00196]$). Graph-construction changes therefore do not measurably mediate answer quality here. This is a mechanistic negative result over 16 cases with two cases per category, not full-corpus invariance.
...
\paragraph{Limitations.} The revised set contains 13 model-constructed and model-validated queries and only four route successfully, so it cannot support a confirmatory answer comparison and we do not offer one. Persona reviews are correlated model simulations, not independent human annotation. The candidate-time issuer extractor is a heuristic; it provably merges distinct companies in 3 of 240 candidates and probably in 2 more (Section~\ref{sec:setup}). The 13 evaluated cases were hand-checked and 12 survive a validated extractor, but the wider pool must be rebuilt before reuse. FinDER categories proxy organizational teams rather than observed enterprise ownership and authorization. Source gold can contain generic inference, and token F1 remains lexical. The exact-token tokenizer is a fixed reproducibility tokenizer, not MARA's internal one. MiniMax often violates the revised JSON contract, and because routing failures were booked separately from schema failures, the schema-failure column is not comparable between the routed and fallback arms. The fallback result is measured on the same 13 cases and inherits their sample-size limits; two of its three answer-level intervals have a lower bound pinned at zero because the arms are identical on the four covered cases. Adversarial results are synthetic and intentionally easy to verify. The mediation gate has two cases per category, so its null indirect effects do not establish full-corpus invariance. Dropout reformation is tested at capability level, not end to end. Finally, the system assumes an externally correct authorization service and does not solve policy administration.
