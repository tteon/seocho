...
def main() -> None:
    ...
    ap.add_argument(
        "--model", default="DeepSeek-V3.1"
    )  # reliable, 0 parse errors in ADR-0115
    ...
    profile = build_corpus_profile(
        graphs, source=f"FinDER open-extract N={len(docs)} {args.model}"
    )
    top = sorted(profile.label_frequencies.items(), key=lambda kv: -kv[1])[:15]
    print(
        f"corpus profile: {len(profile.label_frequencies)} distinct types; top: {top[:8]}"
    )
    ...
    for name, path in VARIANTS.items():
        ...
        a = score_ontology(
            o, competency_questions=cqs, corpus_profile=profile, profile="guardrail"
        )
