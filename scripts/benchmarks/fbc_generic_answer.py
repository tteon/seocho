...
def main() -> None:
    ...
    print(
        ...
        f"fibo_fbc_generic={len(fbc_generic.nodes)} cls",
        flush=True,
    )
    ...
    for name, r in reps.items():
        print(
            f"{name:16s} acc={r.accuracy} by={r.by_category} n={r.n_scored} err={r.errors}",
            flush=True,
        )
