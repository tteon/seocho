...
def mcnemar(b, c):
    ...
    return {
        ...
        "c": c,
        "chi2": round(chi2, 4),
        "p_value": round(math.erfc(math.sqrt(chi2 / 2.0)), 4),
    }
...
def build_fbc_stable_generic(catalog_path, corpus_path, key):
    ...
    bes = [
        create_llm_backend(provider="mara", model=m, api_key=key) for m in DERIVE_MODELS
    ]
    seed = derive_fibo_roots_stable(
        gterms, fbc, backends=bes, models=DERIVE_MODELS, passes=2
    )
    bridged = semantic_bridge(bridge_to_corpus(fbc, cp), seed)
    generic = set(cp.label_frequencies) | set(seed)
    terms = sorted(
        {a for nd in bridged.nodes.values() for a in nd.aliases if a in generic}
    )
    onto = Ontology(
        "fibo_fbc_stable",
        package_id="fibo.FBC.stable",
        version=catalog_provenance(cat)["fibo_commit"][:12],
        nodes={
            t: NodeDef(description=f"{t} (FIBO-FBC stable-auto derived).")
            for t in terms
        },
    )
...
def main():
    ...
    print(
        "building stable-bridged FBC generic guardrail (multi-model derive)...",
        flush=True,
    )
    ...
    ontos = {
        ...
        "fibo_fbc_stable": fbc_stable,
    }
    print(
        f"fibo_fbc_stable = {len(terms)} generic terms; seed={json.dumps(seed)[:300]}",
        flush=True,
    )
    ...
    print(
        f"{len(cases)} cases; curated_plus={len(ontos['curated_plus'].nodes)} cls, fibo_fbc_stable={len(fbc_stable.nodes)} cls",
        flush=True,
    )
    ...
    scored = [
        r
        for r in results
        if "error" not in r and "curated_plus" in r and "fibo_fbc_stable" in r
    ]

    def acc(arm, k):
        return (
            round(sum(1 for r in scored if r[arm][k]) / len(scored), 4)
            if scored
            else 0.0
        )
    ...
    c = sum(1 for a, x in zip(A, B) if x and not a)
    ...
    summary = {
        ...
        "fibo_fbc_stable_terms": terms,
        "derive_models": DERIVE_MODELS,
        "curated_plus": {
            "consensus_acc": acc("curated_plus", "consensus"),
            "j1": acc("curated_plus", "j1"),
            "j2": acc("curated_plus", "j2"),
        },
        "fibo_fbc_stable": {
            "consensus_acc": acc("fibo_fbc_stable", "consensus"),
            "j1": acc("fibo_fbc_stable", "j1"),
            "j2": acc("fibo_fbc_stable", "j2"),
        },
        "accuracy_delta_consensus": round(
            acc("fibo_fbc_stable", "consensus") - acc("curated_plus", "consensus"), 4
        ),
        "mcnemar": mcnemar(b, c),
        "bootstrap_95ci_delta": bootstrap_ci(diffs),
        "inter_judge_agreement": (
            round(sum(1 for x, y in jp if x == y) / len(jp), 4) if jp else None
        ),
    }
