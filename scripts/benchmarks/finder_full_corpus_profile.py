...
_OPEN_SYS = (
    "You extract entities from financial text. For each entity give a short, GENERAL type "
...
def open_extract_all(jsonl_path, *, model, workers, max_chars, resume):
    ...
    print(
        f"total in corpus minus done = {len(docs)} to extract ({len(done_ids)} resumed)",
        flush=True,
    )
        ...
        try:
            ex = _retry(
                lambda: structured_complete(
                    be,
                    system=_OPEN_SYS,
                    user=_OPEN_USER.format(doc=doc["text"]),
                    model=model,
...
def main():
    ...
    ap.add_argument(
        "--skip-extract",
        action="store_true",
        help="only aggregate + score from existing jsonl",
    )
    args = ap.parse_args()
    ...
    cp = CorpusProfile(
        label_frequencies={str(k): int(v) for k, v in freqs.items()},
        doc_count=ndoc,
        source="FinDER full open-extraction",
    )
    ...
    if Path(cat_path).exists():
        ...
        gterms = sorted(cp.label_frequencies, key=lambda k: -cp.label_frequencies[k])[
            :20
        ]
        key = re.search(
            r'ontology_guardrail_mara_api_key\s*=\s*"([^"]+)"', Path(".env").read_text()
        ...
        bes = [
            create_llm_backend(provider="mara", model=m, api_key=key)
            for m in ["DeepSeek-V3.1", "MiniMax-M2.5", "gpt-oss-120b"]
        ]
    ...
    def cov(o):
        d = score_ontology(o, corpus_profile=cp, profile="guardrail").dimension(
            "corpus_coverage"
        )
    ...
    out = {
        "experiment": "finder-full-corpus-profile",
        "doc_count": ndoc,
        "distinct_labels": len(freqs),
        "numeric_intensity": numeric_intensity(cp),
        "top_labels": top,
        "coverage": coverage,
        "chosen": rec.chosen,
    ...
    print(f"\n[written] {args.out}")
    print(
        f"docs={ndoc} labels={len(freqs)} numeric_intensity={out['numeric_intensity']}"
    )
