    ...
    for cat, group in df.sort_values("_id").groupby("category"):
        ...
        for _, row in group.iterrows():
            ...
            if len(text) < 80:
                continue
            docs.append(
                {"id": str(row["_id"]), "category": str(cat), "text": text[:max_chars]}
            )
            taken += 1
...
def _aggregate(per_doc):
    ...
    return {
        ...
        "mean_label_conformance": round(
            statistics.mean([d["label_conformance"] for d in ok]), 4
        ),
        "mean_rel_conformance": round(
            statistics.mean([d["rel_conformance"] for d in ok]), 4
        ),
        "mean_extraction_score": round(
            statistics.mean([d["extraction_score"] for d in ok]), 4
        ),
...
def main() -> None:
    ...
    scorecards = {
        arm: score_ontology(o, competency_questions=cqs).to_dict()
        for arm, o in ontos.items()
    }
    ...
    backends = {
        m: create_llm_backend(provider="mara", model=m, api_key=key) for m in MODELS
    }
    tasks = [
        (arm, m, di, doc)
        for arm in ontos
    ...
    def run_task(t):
        ...
        try:
            r = backends[m].complete(
                ...
                user=extraction_prompt(ontos[arm], doc["text"]),
                temperature=0.0,
                max_tokens=4096,
    ...
    for arm in ontos:
        ...
        for m in MODELS:
            ...
            print(
                f"[{arm} / {m}] nodes={agg.get('mean_nodes')} conf={agg.get('mean_label_conformance')} "
    ...
    for arm in ontos:
        ...
        summary[arm] = (
            {k: round(statistics.mean([a[k] for a in aggs]), 4) for k in keys}
            if aggs
            else {}
        )
    if all(summary.values()):
        summary["delta_B_minus_A"] = {
            k: round(
                summary["B_rich_fibo_plus"][k] - summary["A_sparse_fibo_minus"][k], 4
            )
            for k in keys
        }
    ...
    if "delta_B_minus_A" in summary:
        a, b, d = (
            summary["A_sparse_fibo_minus"],
            summary["B_rich_fibo_plus"],
            summary["delta_B_minus_A"],
        )
        print(f"\nCROSS-MODEL MEANS:")
        print(
            f"  A (sparse): nodes={a['mean_nodes']} conf={a['mean_label_conformance']} score={a['mean_extraction_score']} entropy={a['label_entropy_bits']} distinct={a['distinct_labels']}"
        )
        print(
            f"  B (rich)  : nodes={b['mean_nodes']} conf={b['mean_label_conformance']} score={b['mean_extraction_score']} entropy={b['label_entropy_bits']} distinct={b['distinct_labels']}"
        )
        print(
            f"  Δ(B-A)    : nodes={d['mean_nodes']:+} conf={d['mean_label_conformance']:+} score={d['mean_extraction_score']:+} entropy={d['label_entropy_bits']:+}"
        )
