...
cases = load_finder_cases(
    ROOT / "examples/finder/datasets/finder_tutorial_subset.json"
)[:4]
...
def run_arm(name, routing_on, db):
    ...
    client = Seocho.local(
        ...
        neo4j_user="neo4j",
        neo4j_password=os.getenv("NEO4J_PASSWORD", "seocho-dev"),
        api_key=key,
    )
    ...
    try:
        ...
        try:
            gs.query(
                "MATCH (n) WHERE n._source_id IS NOT NULL DETACH DELETE n", database=db
            )
        except Exception:
            pass
        client.close()
...
for r in (a, b):
    print(
        f"  [{r['arm']:12s}] calls={r['model_calls']}  rel_cost={r['relative_cost']:.0f}"
    )
    print(
        f"                 contains={r['contains_rate']:.2f}  numeric_recall={r['numeric_recall']:.2f}  p50={r['p50_ms']:.0f}ms"
    )
...
print(
    f"  support parity: contains {a['contains_rate']:.2f} -> {b['contains_rate']:.2f}, "
