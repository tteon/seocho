...
ALLOWED_UNITS = {"usd", "$", "dollars", "percent", "%", "shares", "ratio", "x", "eur", "gbp", "jpy", ""}
ALLOWED_SCALES = {"", "ones", "thousand", "thousands", "million", "millions", "billion", "billions"}
...
def fin_numeric_ontology() -> Ontology:
    ...
    return Ontology("fin-numeric", version="1.0.0", nodes={
        ...
        "FinancialMetric": NodeDef(
            ...
            properties={
                "name": P(str, required=True, description="Metric name, e.g. 'total revenue'."),
                ...
                "period": P(str, required=True, description="Fiscal period, e.g. FY2023 or Q2-2022."),
                "company": P(str, description="Reporting company."),
            }),
    }, relationships={
        "REPORTED": RelDef(source="Company", target="FinancialMetric", cardinality="ONE_TO_MANY",
...
def numeric_rules(facts) -> list:
    ...
    for i, f in enumerate(facts):
        if not isinstance(f, dict):
            flags.append(f"fact[{i}] not an object"); continue
        name = str(f.get("name", "")) or f"fact[{i}]"
        v = f.get("value")
        try:
            fv = float(str(v).replace(",", "").replace("$", "").replace("%", "")) if v not in (None, "") else None
        except Exception:
            fv = None
        ...
        nm = name.lower()
        if fv is not None and fv < 0 and any(k in nm for k in ("revenue", "assets", "shares", "cash")):
...
_EXTRACT_SYS = ("You are a financial analyst. Extract the numeric facts needed, then answer the "
...
def main() -> None:
    ...
    def run(case):
        ...
        try:
            r = be.complete(system=_EXTRACT_SYS, user=_EXTRACT_USER.format(refs=case["refs"], q=case["q"]),
                            temperature=0.0, max_tokens=4096, response_format={"type": "json_object"})
            ex = _json(r.text)
        except Exception as e:
            out["error"] = f"extract: {e}"; return out
        ...
        # correctness judge
        try:
            jc = _json(be.complete(system=_JUDGE_SYS, user=_CORRECT_USER.format(q=case["q"], gold=case["gold"], ans=ans),
                                   temperature=0.0, max_tokens=2048, response_format={"type": "json_object"}).text)
            out["correct"] = bool(jc.get("correct"))
        except Exception as e:
            out["error"] = f"judge: {e}"; return out
        # classify wrong errors
        if not out["correct"]:
            try:
                jt = _json(be.complete(system=_JUDGE_SYS,
                                       user=_CLASSIFY_USER.format(q=case["q"], gold=case["gold"], ans=ans,
    ...
    summary = {
        "n_cases": len(cases), "n_scored": len(ok),
        "numeric_accuracy": round(len(correct) / len(ok), 4) if ok else 0.0,
        "n_wrong": len(wrong),
        "wrong_structural": len(structural), "wrong_arithmetic": len(arithmetic),
        "pct_wrong_structural": round(len(structural) / len(wrong), 4) if wrong else 0.0,
        "pct_wrong_arithmetic": round(len(arithmetic) / len(wrong), 4) if wrong else 0.0,
        "shacl_catch_rate_on_structural": round(len(catch_structural) / len(structural), 4) if structural else None,
        "validator_false_positive_rate_on_correct": round(len(fp) / len(correct), 4) if correct else None,
        "by_type_accuracy": {t: round(statistics.mean([1 if r["correct"] else 0 for r in ok if r["type"] == t]), 3)
                             for t in NUMERIC_TYPES if any(r["type"] == t for r in ok)},
    }
    record = {"experiment": "P3-shacl-numeric-validation", "model": args.model,
              "ontology": onto.name, "summary": summary, "results": results}
    Path(args.out).write_text(json.dumps(record, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    print(f"\n[written] {args.out}\n")
    print(f"numeric accuracy: {summary['numeric_accuracy']}  ({len(correct)}/{len(ok)})")
    print(f"wrong answers: {len(wrong)} → structural {summary['pct_wrong_structural']:.0%} / arithmetic {summary['pct_wrong_arithmetic']:.0%}")
    print(f"SHACL+rules CATCH RATE on structural-wrong: {summary['shacl_catch_rate_on_structural']}")
    print(f"validator FALSE-POSITIVE rate on correct answers: {summary['validator_false_positive_rate_on_correct']}")
