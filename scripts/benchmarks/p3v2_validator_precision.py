...
def old_validator_flags(facts) -> bool:
    ...
    for f in facts:
        ...
        try:
            ...
            return True
        if (
            str(f.get("unit", "")).strip().lower()
            and str(f.get("unit", "")).strip().lower() not in _OLD_UNITS
        ):
            return True
        if (
            str(f.get("scale", "")).strip().lower()
            and str(f.get("scale", "")).strip().lower() not in _OLD_SCALES
        ):
            return True
...
def main():
    ...
    def rate(subset, key):
        return (
            round(sum(1 for r in subset if r.get(key)) / len(subset), 4)
            if subset
            else None
        )
    ...
    summary = {
        ...
        "OLD": {
            "recall_on_structural": rate(structural, "old_flagged"),
            "false_positive_on_correct": rate(correct, "old_flagged"),
        },
        "NEW": {
            "recall_on_structural": rate(structural, "new_flagged"),
            "false_positive_on_correct": rate(correct, "new_flagged"),
        },
    }
    rec = {
    ...
    print(f"\n[written] {args.out}\n")
    print(
        f"scored={len(scored)} correct={len(correct)} structural_wrong={len(structural)}"
    )
    print(
        f"OLD: recall={summary['OLD']['recall_on_structural']} FP_on_correct={summary['OLD']['false_positive_on_correct']}"
    )
    print(
        f"NEW: recall={summary['NEW']['recall_on_structural']} FP_on_correct={summary['NEW']['false_positive_on_correct']}"
    )
