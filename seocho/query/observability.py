...
def usage_from_response(response: Any, *, model: str = "", cost_usd: float | None = None) -> LLMUsage:
    """Normalize dict/object LiteLLM responses without importing LiteLLM."""

    usage: Any = response.get("usage", {}) if isinstance(response, Mapping) else getattr(response, "usage", {})
    if usage is None:
        usage = {}

    def read(name: str, fallback: int = 0) -> int:
        value = usage.get(name, fallback) if isinstance(usage, Mapping) else getattr(usage, name, fallback)
        try:
    ...
    resolved_model = model or (response.get("model", "") if isinstance(response, Mapping) else getattr(response, "model", ""))
    return LLMUsage(
