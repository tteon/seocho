...
_RE_COMPANY = re.compile(r"\b(corporation|corp|company|co|incorporated)\b\.?", flags=re.IGNORECASE)
_RE_SENTENCE_SPLIT = re.compile(r"(?<=[.!?])\s+")
_RE_ALPHA_NUM_TOKEN = re.compile(r"[a-z0-9]+")
...
class AnswerGenerationAgent:
    ...
    def _direct_answer(question: str, supporting_fact: str) -> str:
        ...
        question_terms = {
            token
            for token in _RE_ALPHA_NUM_TOKEN.findall( question.lower())
        ...
        def score(sentence: str) -> tuple[int, int, int]:
            terms = set(_RE_ALPHA_NUM_TOKEN.findall( sentence.lower()))
