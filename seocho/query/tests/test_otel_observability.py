...
def test_bridge_updates_fake_meter_instruments() -> None:
    class Instrument:
        def __init__(self):
            self.values = []
        def add(self, value, attributes=None): self.values.append((value, attributes))
        def record(self, value, attributes=None): self.values.append((value, attributes))
    class Meter:
        def __init__(self): self.instruments = []
        def create_counter(self, _name):
            item = Instrument(); self.instruments.append(item); return item
        def create_histogram(self, _name):
            item = Instrument(); self.instruments.append(item); return item
    meter = Meter(); bridge = OTelBridge(meter=meter)
