...
from .client_execution import ExecutionPlanBuilder as ExecutionPlanBuilder
...
if TYPE_CHECKING:
    from .client_async import AsyncSeocho as AsyncSeocho
...
class Seocho:
    ...
    def index_file(
        ...
        *,
        ...
        validates, and writes to the graph. This single-file SDK entrypoint
        indexes on every call. Use index_directory() for mtime/size tracking.
    ...
    def execute(self, plan: ExecutionPlan | Dict[str, Any]) -> ExecutionResult:
        ...
        semantic_result = self.semantic(
            ...
            query_mode=resolved_plan.reasoning.query_mode,
            reasoning_cycle=resolved_plan.reasoning.reasoning_cycle or None,
        )
        return ExecutionResult.from_run_result(
    ...
    def _execute_local_plan(self, plan: ExecutionPlan) -> ExecutionResult:
        ...
        response = self.ask(
            ...
            query_mode=plan.reasoning.query_mode,
        )
...
def __getattr__(name: str) -> Any:  # noqa: D401  (module-level dunder)
    if name in {"AsyncSeocho", "_fill_async_surface"}:
        from . import client_async

        return getattr(client_async, name)
