"""Backward-compatible alias — canonical location is ``seocho.ontology.context``.

Kept because `seocho.ontology_context` is an import path users, docs and tests already use.
...
``seocho.ontology_context is seocho.ontology.context`` — one module object, not two.
...
from .ontology import context as _canonical
