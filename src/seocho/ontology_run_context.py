"""Backward-compatible alias — canonical location is ``seocho.ontology.run_context``.

Kept because `seocho.ontology_run_context` is an import path users, docs and tests already use.
...
``seocho.ontology_run_context is seocho.ontology.run_context`` — one module object, not two.
...
from .ontology import run_context as _canonical
