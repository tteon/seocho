"""Backward-compatible alias — canonical location is ``seocho.ontology.serialization``.

Kept because `seocho.ontology_serialization` is an import path users, docs and tests already use.
...
``seocho.ontology_serialization is seocho.ontology.serialization`` — one module object, not two.
...
from .ontology import serialization as _canonical
