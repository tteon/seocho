"""Backward-compatible alias — canonical location is ``seocho.ontology.slice``.

Kept because `seocho.ontology_slice` is an import path users, docs and tests already use.
...
``seocho.ontology_slice is seocho.ontology.slice`` — one module object, not two.
...
from .ontology import slice as _canonical
