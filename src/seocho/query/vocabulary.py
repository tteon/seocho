"""Canonical approved vocabulary resolution; no runtime-store dependency."""
...
from .artifact_reader import DEFAULT_SEMANTIC_ARTIFACT_DIR, list_artifacts, get_artifact
...
class ManagedVocabularyResolver:
    ...
    def __init__(
        ...
        *,
        ...
        self.base_dir = base_dir or os.getenv(
            "SEMANTIC_ARTIFACT_DIR", DEFAULT_SEMANTIC_ARTIFACT_DIR
        )
        configured_global = os.getenv(
        ...
        if isinstance(vocab_candidate, dict):
            for term in vocab_candidate.get("terms", []):
                ...
                if not isinstance(hidden_labels, list):
                    hidden_labels = []
                self._register_term(
                    aliases, canonical, [*term_aliases, *alt_labels, *hidden_labels]
                )
        ...
        if isinstance(shacl_candidate, dict):
            for shape in shacl_candidate.get("shapes", []):
                if not isinstance(shape, dict):
                    continue
                self._register_term(
                    aliases, str(shape.get("target_class", "")).strip(), []
                )
    ...
    def _list_semantic_artifacts(
        ...
        return list_artifacts(self.base_dir, workspace_id, status=status)

    def _get_semantic_artifact(
        self, workspace_id: str, artifact_id: str
    ) -> Dict[str, Any]:
        return get_artifact(self.base_dir, workspace_id, artifact_id)
